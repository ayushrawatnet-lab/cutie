...
function draw(now){ ctx.clearRect(0,0,innerWidth,innerHeight); for(const p of particles){ p.y += p.vy; if(p.y < -15) p.y = innerHeight+15; if(p.y > innerHeight+15) p.y = -15; ctx.globalAlpha = p.a * (.7 + .3*Math.sin(now*.001+p.x)); if(p.type==='star'){ctx.fillStyle='#fff';ctx.fillRect(p.x,p.y,p.r,p.r)} else {ctx.fillStyle=p.type==='heart'?'#ff8db5':'#d99bae';ctx.font=`${p.type==='heart'?9:7}px serif`;ctx.fillText(p.type==='heart'?'♥':'✦',p.x,p.y)} } ctx.globalAlpha=1; requestAnimationFrame(draw); }
